...
const ModelEditor = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        var emptyFieldNames = Object.keys(inputFields).filter((id) => {
            if (inputFields[id].fieldName.length === 0) console.log('empty field found!');
        ...
        var emptyFieldDatatypes = Object.keys(inputFields).filter((id) => {
            if (inputFields[id].fieldDatatype.length === 0) console.log('empty fieldDatatype found!');
            return (inputFields[id].fieldDatatype.length === 0);
        });
        if (emptyFieldDatatypes.length > 0) {
            alert(`Please complete all fieldDatatype. The Field datatypes not completed are: ${JSON.stringify(emptyFieldDatatypes)}`)
            return;
        }
    ...
    const handleAddFields = () => {
        setInputFields([...inputFields, { fieldName: "", fieldDatatype: "", fieldNameError: "" }]);
        };
    ...
    <div className="ModelEditor">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div key={index} className="flex items-center space-x-2">
                    ...
                    onValueChange={(value) => handleSelectChangeInput(index, value)}
                    className="form-input px-4 py-2 rounded border"
                    >
                        {fieldDatatypeOptions.map((fieldDatatype, selectIndex) => (
                        <SelectItem key={selectIndex} value={fieldDatatype}>{fieldDatatype}</SelectItem>
                        ))}
                    </Select>
